...
function Download-FileFromWeb {
    ...
    If (!(Test-Path $SaveFolder)) {
        Write-Output "Creating directory: $SaveFolder"
...
function Improve-SSDSettings {
    Write-Output "Applying SSD optimization settings..."
...
function Create-SystemRestorePoint {
    ...
    Write-Output "Creating system restore point: $RestorePointName"
...
function Run-SystemDebloat {
    ...
    If (!$UndoChanges) {
        ...
        Write-Output "Launching Malwarebytes AdwCleaner..."
    ...
    Push-Location -Path (Split-Path -Path $ShutUp10Path)

    If ($UndoChanges) {
        Write-Output "Reverting settings using O&O ShutUp10++..."
        Start-Process -FilePath "$ShutUp10Path" -ArgumentList "ooshutup10-default.cfg", "/quiet" -Wait
    } Else {
        Write-Output "Applying recommended settings with O&O ShutUp10++..."
        Start-Process -FilePath "$ShutUp10Path" -ArgumentList "ooshutup10.cfg", "/quiet" -Wait
    }

    Pop-Location
    Remove-Item -Path (Split-Path -Path $ShutUp10Path) -Force -Recurse
}
...
function TurnOff-MouseAcceleration {
    Write-Output "Turning off mouse acceleration..."
...
function TurnOn-MouseAcceleration {
    Write-Output "Turning on mouse acceleration..."
...
function Hide-NewsAndInterests {
    Write-Output "Disabling 'News and Interests' on the taskbar..."
...
function Show-NewsAndInterests {
    Write-Output "Enabling 'News and Interests' on the taskbar..."
...
function Configure-OptionalFeatures {
    ...
    foreach ($Feature in $FeaturesList) {
        ...
        if ($FeatureInfo) {
            ...
            Write-Output "Feature not found: $Feature"
        }
    }
}
...
# Performance tweaks
Write-Output "Applying performance enhancements..."
...
function Optimize-SystemServices {
    ...
    Write-Output "Adjusting Windows services for optimal performance..."
...
function Optimize-ScheduledTasks {
    ...
    Write-Output "Modifying scheduled tasks for better performance..."
...
# Final message
Write-Output "System optimization script has completed successfully!"
